...
print("="*80)
print("MULTI-TARGET AIR QUALITY PREDICTION - RANDOM FOREST")
print("="*80)
...
for target_name, target_col in target_columns.items():
    ...
    joblib.dump(model, f'models_rf/{target_name.lower().replace(".", "")}_model.pkl')
    joblib.dump(scaler, f'models_rf/{target_name.lower().replace(".", "")}_scaler.pkl')
...
summary_df.to_csv('models_rf/performance_summary_rf.csv', index=False)
...
for idx, (name, res) in enumerate(all_results.items()):
    ...
    axes[idx, 0].set_title(f'{name} - Random Forest (R²: {res["test_r2"]:.3f})')
