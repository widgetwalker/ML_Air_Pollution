...
print("="*80)
print("MULTI-TARGET AIR QUALITY PREDICTION - RIDGE REGRESSION")
print("="*80)
...
for target_name, target_col in target_columns.items():
    ...
    for train_idx, val_idx in tscv.split(X_train_scaled):
        cv_model = Ridge(alpha=1.0, random_state=42)
    ...
    joblib.dump(model, f'models_ridge/{target_name.lower().replace(".", "")}_model.pkl')
    joblib.dump(scaler, f'models_ridge/{target_name.lower().replace(".", "")}_scaler.pkl')
...
summary_df.to_csv('models_ridge/performance_summary_ridge.csv', index=False)
...
for idx, (name, res) in enumerate(all_results.items()):
    ...
    axes[idx, 0].set_title(f'{name} - Ridge (R²: {res["test_r2"]:.3f})')
...
plt.savefig('graphs/model_evaluations/evaluation_ridge.png', dpi=300, bbox_inches='tight')
print("\n✓ Saved: graphs/model_evaluations/evaluation_ridge.png")
print(f"✓ Trained {len(all_results)} Ridge models")
