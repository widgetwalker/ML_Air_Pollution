# Batch script to create all remaining ML model training scripts
# This creates: Lasso, SVR, KNN, Decision Tree, and ElasticNet

import os

# Template for model training scripts
def create_model_script(model_name, model_import, model_class, model_params, dir_suffix):
    template = f'''
import pandas as pd
import numpy as np
...
print("="*80)
print("MULTI-TARGET AIR QUALITY PREDICTION - {model_name.upper()}")
print("="*80)
...
os.makedirs('models_{dir_suffix}', exist_ok=True)
all_results = {{}}

print("\\n[STEP 3] Training {model_name} models...")
for target_name, target_col in target_columns.items():
    ...
    joblib.dump(model, f'models_{dir_suffix}/{{target_name.lower().replace(".", "")}}_model.pkl')
    joblib.dump(scaler, f'models_{dir_suffix}/{{target_name.lower().replace(".", "")}}_scaler.pkl')
...
summary_df.to_csv('models_{dir_suffix}/performance_summary_{dir_suffix}.csv', index=False)
...
for idx, (name, res) in enumerate(all_results.items()):
    ...
    axes[idx, 0].set_title(f'{{name}} - {model_name} (R²: {{res["test_r2"]:.3f}})')
...
plt.tight_layout()
plt.savefig('evaluation_{dir_suffix}.png', dpi=300, bbox_inches='tight')
print("\\n✓ Saved: evaluation_{dir_suffix}.png")
print(f"✓ Trained {{len(all_results)}} {model_name} models")
'''
    return template

# Model configurations
models = [
    ("Lasso", "from sklearn.linear_model import Lasso", "Lasso", "alpha=1.0, random_state=42", "lasso"),
    ...
    ("ElasticNet", "from sklearn.linear_model import ElasticNet", "ElasticNet", "alpha=1.0, l1_ratio=0.5, random_state=42", "elasticnet"),
]

# Create all scripts
for model_name, model_import, model_class, model_params, dir_suffix in models:
    filename = f"train_{dir_suffix}.py"
    script_content = create_model_script(model_name, model_import, model_class, model_params, dir_suffix)
...
print("\n" + "="*80)
print("ALL MODEL TRAINING SCRIPTS CREATED!")
print("="*80)
print("\nCreated files:")
print("  - train_lasso.py")
print("  - train_svr.py")
print("  - train_knn.py")
print("  - train_dt.py")
print("  - train_elasticnet.py")
